...
export default function BenzerKararlarClient({
    item,
    kararlar,
}) {
    console.log("BENZER:", kararlar);
    console.log("LENGTH:", kararlar?.length);
        ...
        <div className="rounded-2xl border border-white/10 bg-[#0d1320] p-6">
            ...
            <div className="mt-10 flex items-center justify-between rounded-3xl border border-white/10 bg-white/[0.04] p-4">
                <button
                    ...
                    disabled={page === 1}
                    className="rounded-2xl border border-white/10 px-5 py-3 text-sm text-slate-300 disabled:opacity-40"
                >
                    ← Önceki
                </button>
